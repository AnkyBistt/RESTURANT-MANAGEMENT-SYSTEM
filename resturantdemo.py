...
connection.execute("CREATE TABLE IF NOT EXISTS"+TABLE_NAME+"("+TABLE_ID+"INTEGER PRIMARY KEY AUTOINCREMENT ,"+CHICKEN_ID+", INTEGER,"+CHAPPATI_ID+" INTEGER,"+DAAL_ID+", INTEGER"+VEGETABLE_ID+" INTEGER,"+DRINKS_ID+",INTEGER"+DESERT_ID+",INTEGER"+SNACK_ID+",INTEGER"+COST_ID+",INTEGER"+SERVICE_ID+",INTEGER"+GST_ID+",INTEGER"+TOTAL_ID+",INTEGER);")
...
def takeInput():
    ...
    connection.execute("INSERT INTO"+TABLE_NAME+ "("+CHICKEN_ID+","+CHAPPATI_ID+","+DAAL_ID+","+VEGETABLE_ID+","+DRINKS_ID+","+DESERT_ID+","+SNACK_ID+","+COST_ID+","+SERVICE_ID+","+GST_ID+","+TOTAL_ID+")VALUES("+str(chickenItem)+ ","+str(chappatiItem)+","+str(daalItem)+","+str(vegetableItem)+","+str(drinkItem)+","+str(desertItem)+","+str(snackItem)+","+str(costItem)+","+str(serviceItem)+","+str(gstItem)+","+str(totalItem)+");")
    connection.commit()
...
i=0
for row in cursor:
    tree.insert('',i,text="FOOD ITEM"+str(row[0])),values(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11])
    i=i+1
tree.pack()
connection.close()
window.mainloop()
